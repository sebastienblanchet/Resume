...
\tb Programmed LabVIEW HMI for communication between PC, PXIe RTOS controller and FPGA models \\
\tb Converted MATLAB/Simulink algorithms to discrete firmware in C and models on FPGA\\
\tb Optimized FPGA models for high fidelity, low latency $\mu$s control \\
\tb Emulated control system plant model on \textbf{deterministic} NI cRIO Linux controller \\
\tb Applied DSP theory through data acquistion with discrete filters in C \textbf{anti-aliasing filters} \\
% \tb Acquire and generate analog and digital signals, control timing, and implement signal processing on RT and FPGA \\
\tb Debug and bring-up of PCBAs with in-house test racks, electronic lab equipment and NI instrumentation \\
